...
uint64
sys_open_symlink(struct inode *ip, uint tolerance, int omode) {
  // note that ip is locked
  if (tolerance == 0) {
    iunlock(ip);
    return -1;
  }

  int nofollow = omode & O_NOFOLLOW;
  if (nofollow) {
    struct file *f;  // file object
  ...
  pack.path_[pack.len_] = '\0';
  struct inode *ip2 = namei(pack.path_);
  if (ip2 == 0) {
    return -1;
  }
  ...
  ilock(ip2);
  if (ip2->type == 0) {
    panic("sys_open_symlink: untyped");
  }
  if (ip2->type == T_DIR || ip2->type == T_DEVICE) {
    // cannot have symbolic links to directories
    iunlockput(ip2);
    end_op();
    return -1;
  }
  ...
  // ip2->type == T_SYMLINK
  return sys_open_symlink(ip2, tolerance - 1U, omode);
}
