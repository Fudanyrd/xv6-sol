...
extern void
safe_uvmunmap(pagetable_t pagetable, uint64 va, uint64 npages, int do_free);

// notice that len is page aligned in mmaptest, 
// and offset is always zero,
// so we can make some safe optimizations.
uint64
sys_mmap(void) {
  uint64 addr;
